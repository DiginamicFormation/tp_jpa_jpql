...
public class ActeurRepositoryTest {
	...
	public void testExtraireActeursParRole() {
		...
		TypedQuery<Acteur> query = em.createQuery("SELECT a FROM Acteur a JOIN a.roles r WHERE r.nom = 'Harley QUINN'",
				Acteur.class);
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireActeursParFilmParuAnnee() {
		TypedQuery<Acteur> query = em
				.createQuery("SELECT a FROM Acteur a JOIN a.roles r JOIN r.film f WHERE f.annee = 2015", Acteur.class);
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireActeursParListePaysEtAnnee() {
		TypedQuery<Acteur> query = em.createQuery(
				"SELECT DISTINCT a FROM Acteur a JOIN a.roles r JOIN r.film f JOIN  f.pays p WHERE p.nom= 'france' and f.annee = 2017",
				Acteur.class);
		List<Acteur> acteurs = query.getResultList();
	...
	public void testExtraireParRealisateurEntreAnnee() {
		TypedQuery<Acteur> query = em.createQuery(
				"SELECT DISTINCT a FROM Acteur a JOIN a.roles r JOIN r.film f JOIN f.realisateurs r WHERE f.annee BETWEEN 2010 AND 2020 AND r.identite = 'Ridley Scott'",
				Acteur.class);
		List<Acteur> acteurs = query.getResultList();
